...
class SafelyApi extends CI_Controller
{
    ...
    {header( 'Content-type: text/html' );
        echo 'Welcome to Safely apis';
		        echo '<br><b>Base url : https://project.cyphersol.com/safelify/safelyApi</b><br> 
				<a href="https://documenter.getpostman.com/view/8463706/2s7Z7ZoZpb"> postman document</a>';
    }
	...
	$planinfo=  $this->db->where( 'id', $plan_id )->get('plans')->row();
	  
$plan_code=$planinfo->paystack_plan_code;
...
$amount=$plan_price*100;
 ...
 $object= json_decode($result);
  if(isset($object->status)){
	   $this->data['message'] = $object->message;
       $this->data['authorization_url'] = $object->data->authorization_url;
       $this->data['reference'] = $object->data->reference;
	   
	   // insert in database 
	   $this->db->insert('subscriptions',array('plan_id'=>$plan_id,'user_id'=>USER_ID,'reference'=>$object->data->reference,'authorization_url'=>$object->data->authorization_url,'type'=>'paystack'));
 ...
 // pre($object);
  if(isset($object->data->status) and $object->data->status=='success'){
      $this->db->where('reference',$_GET['reference'])->update('subscriptions',array('body'=>$response));
    $planinfo=  $this->db->where('reference',$_GET['reference'])->get('subscriptions')->row();
    
    // update user plan_id
    $this->db->where('id',$planinfo->user_id)->update('users',array('plan_id'=>$planinfo->plan_id));
      echo '<html>
  <head>
    ...
    <body>
      ...
      <div style="border-radius:200px; height:200px; width:200px; background: #F8FAF5; margin:0 auto;">
        ...
        <h1>Success</h1> 
        <p>You have successfullt subscribed;<br/> Thanks!</p>
      </div>
    </body>
</html>';
  }else{
 ...
 // pre($object);
  if(isset($object->data->status) and $object->data->status=='success'){
    ...
    // update user plan_id
    $this->db->where('id',$planinfo->user_id)->update('users',array('plan_id'=>$planinfo->plan_id));
    $plan=  $this->db->where('id',$planinfo->plan_id)->get('plans')->row();
    $this->data['status']= 200;
    $this->data['message']= "Success:  Verified & plan is ".$plan->name ;
    $this->response($this->data);
      
  }else{
      
     $this->data['status']= 204;
    $this->data['message']= "Error: Transaction not verified" ;
    $this->response($this->data);
  }
...
$this->db->insert('tbl_payment_notifications',array('body'=>$input,'type'=>'paystack'));
    }
    ...
    function stripePayment( $stripeToken, $amount )
    {
        ...
        try {
            ...
            if ( is_array( $chargeJson ) && $chargeJson['object'] == 'charge' ) {
                ...
                $sellerHtml .= '';
                $sellerHtml .= '<br><br><br><br><a href="' . base_url() . '"  target="_blank">Safely Team </a>';
	...
	$this->data['data']= $this->db->select('`id` as plan_id, `name`, `price`, `interval`, `description`,paystack_plan_code')->get('plans')->result_array();  
	$this->response($this->data);
    }
    ...
    { extract( $_POST );
		$this->AM->verifyRequiredParams(array("report_id"));
		$where=array('c.report_id'=>$report_id);
        $data = $this->db->select(" c.*,u.name,CONCAT('" . base_url() . "uploads/', u.image) as image" )->from('comments as c')
        ->where($where)
		...
		$this->data['data']=$data->result_array();
        $this->response( $this->data );
    }
    ...
    public function viewYourPlan()
    { extract( $_POST );
		$data = $this->db->select("p.name as plan_title" )->from('plans as p')
        ->where(array('u.id'=>USER_ID))
        ->join('users as u','u.plan_id=p.id')
        ->get();
    ...
    { 
		...
		if(isset($_GET['filter']) and $_GET['filter']=='my'){
			$data = $this->db->select(" c.*,CONCAT('" . base_url() . "', c.file) as file,u.name,CONCAT('" . base_url() . "uploads/', u.image) as image" )->from('community_reports as c')
        ->join('users as u','u.id=c.user_id')
        ...
        ->order_by( "c.id", "desc" )->get();
			...
			if($data->num_rows()>0) {
        foreach ( $data->result() as $row ) {
			$row->commentCount=$this->db->where('report_id',$row->id)->count_all_results('comments');
...
$geolocation = $lat.','.$lon;
 ...
 $json = @file_get_contents($url);
	...
	public function reportStatus()
    {
		 $this->checkLogin();
        extract( $_POST );
		if(!in_array($_POST['status'],array('Corroborate','Flag'))){
			$this->error('Possible values are Corroborate,Flag');
			}
        if ( $this->AM->verifyRequiredParams( array(
             "status"
        ) ) ) {
        ...
        if ( checkExist( 'report_votes',$where) == 1 ) {
			if ( $this->db->where($where)->update('report_votes', $_POST ) ) {
                $this->data['message'] = 'Updated & marked as '.$_POST['status'];
                $this->response($this->data);
            }
		}
		
            if ( $this->db->insert( 'report_votes', $_POST ) ) {
                $this->data['message'] = 'Successfully marked as '.$_POST['status'];
                $this->response($this->data);
            }
        }
    }
	...
	public function addContact()
    {
        ...
        if ( checkExist( 'contacts',$_POST) == 1 ) {
			$this->error('Exist already!');
		}
		if(isset($_POST['type']) and $_POST['type']==''){
		    $_POST['type']='other';
		}
			
            if ( $this->db->insert( 'contacts', $_POST ) ) {
	...
	public function editContact()
    {
        ...
        if ( $this->AM->verifyRequiredParams( array(
            ...
            if ( $this->db->where('id',$id)->update( 'contacts', $_POST ) ) {
    ...
    public function emergencyContacts(){ 	
		...
		if(isset($_GET['type'])){
	    $this->data['data']= $this->db->select('emergency_contacts.*,cities.city')
		->join('cities','cities.id=emergency_contacts.city_id')
		->where('type',$_GET['type'])
		->get('emergency_contacts')->result_array();
	}else{
		$this->data['data']= $this->db->select('emergency_contacts.*,cities.city')
		->join('cities','cities.id=emergency_contacts.city_id')
		->get('emergency_contacts')->result_array();
		}
		$this->data['cities']= $this->db->select('id as city_id,city')->get('cities')->result_array();  
	...
	public function editComment()
    {
        ...
        if ( $this->AM->verifyRequiredParams( array(
            ...
            if ( $this->db->where('id',$id)->update( 'comments', $_POST ) ) {
	...
	public function deleteComment()
    {
        ...
        if ( $this->AM->verifyRequiredParams( array(               
			...
			if (! checkExist('comments', $where ) == 1 ) {
				$this->error('You can not delete');
			}
            if ( $this->db->where($where)->delete( 'comments') ) {
                $this->data['message'] = 'Comment Deleted!';
                $this->response($this->data);
            }else{
            ...
            if ( $row->num_rows() > 0 ) {
                if ( $this->ion_auth->login( $email, $password ) ) {
                    ...
                    if ( $user != NULL ) {
                       ...
                       $response['sms_to']    = $this->db->select( 'phone' )->where( 'id', 1 )->get( 'setting' )->row()->phone;
					   $response['share_message']    = $this->db->select( 'share_message' )->where( 'id', 1 )->get( 'setting' )->row()->share_message;
					   
					    $response['cities']=$this->db->query("SELECT city FROM `community_reports` WHERE city !='' GROUP BY city")->result_array();
    ...
    function forgotPassword()
    {
        ...
        if ( $this->form_validation->run() === false ) {
            ...
            if ( $data->num_rows() > 0 ) {
                ...
                ';
                    $this->email->from( 'noreply@Safely.com', 'Safely  ' );
                    $this->email->to( $email );
