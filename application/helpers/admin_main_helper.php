...
function getSidebar(){
	$CI = &get_instance();
	return $CI->load->view("admin/aside");
}
function getFooter(){
	$CI = &get_instance();
...
$key='';
	...
	{
		$CI =& get_instance();
		  $query=$CI->db->query("SELECT n.*,u.name,u.image FROM `notifications` as n 
JOIN users as u on u.id=n.sender_id
WHERE receiver_id=1 and readed=0");
