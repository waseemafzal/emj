...
class States extends MX_Controller {
	...
	public $view = "states_view";
	
	/************Configuration of form and dataTable*********************/
	public $tbl = 'tbl_states';
	...
	public $module_heading='States Management'; 
	/************Configuration of form and End*********************/
	public function index(){  
			$aData['data'] =$this->db->query("SELECT p.* FROM ".$this->tbl." as p where country_id= ".$_GET['country_id']);
			
		$aData['tbl'] =$this->tbl;
		$aData['country_name'] =$this->getCountryById($_GET['country_id']);
		$aData['module_heading'] =$this->module_heading;
		$aData['tableColmns'] =$this->tableColmns;
		$aData['country_id'] =$_GET['country_id'];
		$this->load->view($this->view,$aData);
	}
	public function getCountryById($id){
		return $this->db->select('name')->where('id',$id)->get('tbl_countries')->row()->name;
		}
	public function add(){  
		$aData['tbl'] =$this->tbl;
		...
		$aData['country_id'] =$_GET['country_id'];
		$aData['country_name'] =$this->getCountryById($_GET['country_id']);
		$this->load->view('save_state',$aData);
	}
...
$num = $e['code'];
$mess = $e['message'];
$redirect=base_url().'countries_management/states/index?country_id='.$country_id;
		switch($result){
			case 1:
